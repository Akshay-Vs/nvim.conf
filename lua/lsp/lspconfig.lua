...
return {{
    ...
    config = vim.schedule_wrap(function(_, opts)
        ...
        if opts.inlay_hints.enabled then
            if LazyVim and LazyVim.lsp then
                LazyVim.lsp.on_supports_method("textDocument/inlayHint", function(client, buffer)
                    if vim.api.nvim_buf_is_valid(buffer) and vim.bo[buffer].buftype == "" and
                        not vim.tbl_contains(opts.inlay_hints.exclude, vim.bo[buffer].filetype) then
        ...
        local mason_all = have_mason and
                              vim.tbl_keys(require("mason-lspconfig.mappings").get_mason_map().lspconfig_to_package) or
                              {} --[[ @as string[] ]]
        ...
        if have_mason then
            require("mason-lspconfig").setup({
                ensure_installed = vim.list_extend(install, LazyVim.opts and
